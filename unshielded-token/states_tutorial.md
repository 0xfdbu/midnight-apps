...
By the end, you will have a reusable `useContractState` hook that keeps your UI in sync with on-chain state — whether you prefer polling or push-based subscriptions. This is essential for any dApp dashboard that displays token supply, vault balances, or burn statistics that change after every transaction.
...
This is simpler than querying the contract because the wallet already knows its own balances. You just look up the key matching your stablecoin's token color. If the user has never held the token, the key is absent and you default to `0n`.
...
The `unshielded-token` project implements all three steps in `contractCalls.ts` and `useContractState.ts`. Add a subscription to the mix and you have a dashboard that stays in sync with the chain in real time.
