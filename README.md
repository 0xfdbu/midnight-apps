...
**Features:**
- Mint shielded tokens with `createShieldedToken`
- Atomic mint-and-send with `mintAndSend` / `sendImmediateShielded` (no Merkle path needed)
- Burn shielded tokens via `depositAndBurn` with `receiveShielded`
- Wallet-native shielded transfers via `makeTransfer`
- `localStorage` coin tracking for DApp-minted coins
- Dashboard with shielded balance and contract state (`totalSupply`, `totalBurned`)
